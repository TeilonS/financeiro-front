    ...
    <div className="p-8 max-w-7xl mx-auto min-h-screen bg-zinc-50 dark:bg-[#050505]">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        ...
        <button 
          ...
          className="bg-primary-500 hover:bg-primary-600 text-zinc-900 dark:text-white px-6 py-3 rounded-2xl font-bold text-sm transition-all shadow-lg shadow-primary-500/10 flex items-center gap-2"
        >
      ...
      {investimentos.length > 0 && (
        <div className="bg-primary-500 rounded-3xl p-8 text-zinc-900 dark:text-white relative overflow-hidden shadow-2xl shadow-primary-500/20 mb-10">
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-[#0A0A0A] rounded-3xl border border-zinc-200 dark:border-white/5 text-center py-20 text-zinc-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {investimentos.map(i => (
            <div key={i.id} className="bg-white dark:bg-[#0A0A0A] rounded-3xl border border-zinc-200 dark:border-white/5 p-6 hover:border-white/10 transition-colors group">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-white dark:bg-zinc-900 rounded-2xl flex items-center justify-center text-primary-400 border border-zinc-200 dark:border-white/5">
                    <Landmark size={24} />
                  </div>
                  <div>
                ...
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => openHistory(i)} className="p-2 hover:bg-white/5 rounded-xl text-zinc-500 hover:text-zinc-900 dark:text-white transition-colors">
                    <History size={16} />
                  </button>
              ...
              <div className="space-y-4">
                ...
                <div className="pt-4 border-t border-zinc-200 dark:border-white/5 flex items-center justify-between">
                  <span className="px-3 py-1 bg-white dark:bg-zinc-900 rounded-full text-[10px] font-bold text-zinc-600 dark:text-zinc-400 uppercase tracking-widest border border-zinc-200 dark:border-white/5">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Novo Investimento">
        <form onSubmit={handleSubmit} className="space-y-6 p-2">
          ...
          <button type="submit" disabled={formLoading} className="w-full bg-primary-500 hover:bg-primary-600 text-zinc-900 dark:text-white py-3 rounded-2xl font-bold text-sm transition-all disabled:opacity-50 flex items-center justify-center gap-2">
      ...
      <Modal open={historyModalOpen} onClose={() => setHistoryModalOpen(false)} title="Histórico de Evolução">
        <div className="p-2 space-y-4 max-h-[400px] overflow-y-auto">
          {historyLoading ? (
            ...
            historico.map(s => (
              <div key={s.id} className="flex items-center justify-between p-4 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-white/5">
                <div>
