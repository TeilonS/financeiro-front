...
const inputCls = 'w-full px-4 py-2.5 border border-zinc-300 dark:border-zinc-700 rounded-xl text-sm bg-zinc-50 dark:bg-zinc-800 dark:text-zinc-900 dark:text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500/40 focus:border-primary-500'
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-xl font-semibold text-zinc-900 dark:text-zinc-900 dark:text-white">Cartões de Crédito</h1>
          <p className="text-zinc-400 dark:text-zinc-500 text-sm mt-0.5">Acompanhe faturas e limites</p>
        </div>
        <button onClick={openNew} className="bg-primary-600 hover:bg-primary-700 text-zinc-900 dark:text-zinc-900 dark:text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors flex items-center gap-2">
      ...
      {cartoes.length > 0 && (
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-white dark:bg-white dark:bg-zinc-900 shadow-sm border border-zinc-100 dark:border-zinc-100 dark:border-zinc-800 rounded-2xl border border-zinc-200 dark:border-zinc-100 dark:border-zinc-800 shadow-sm p-5">
            <p className="text-xs font-medium text-zinc-400 dark:text-zinc-500 uppercase tracking-wide mb-1">Total de Faturas</p>
            <p className="text-2xl font-bold text-red-500">{fmt(totalFaturas)}</p>
          </div>
          <div className="bg-white dark:bg-white dark:bg-zinc-900 shadow-sm border border-zinc-100 dark:border-zinc-100 dark:border-zinc-800 rounded-2xl border border-zinc-200 dark:border-zinc-100 dark:border-zinc-800 shadow-sm p-5">
            <p className="text-xs font-medium text-zinc-400 dark:text-zinc-500 uppercase tracking-wide mb-1">Limite Total Disponível</p>
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-white dark:bg-zinc-900 shadow-sm border border-zinc-100 dark:border-zinc-100 dark:border-zinc-800 rounded-2xl border border-zinc-200 dark:border-zinc-100 dark:border-zinc-800 shadow-sm text-center py-16 text-zinc-400 dark:text-zinc-500">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {cartoes.map(c => {
            ...
            return (
              <div key={c.id} className="bg-white dark:bg-white dark:bg-zinc-900 shadow-sm border border-zinc-100 dark:border-zinc-100 dark:border-zinc-800 rounded-2xl border border-zinc-200 dark:border-zinc-100 dark:border-zinc-800 shadow-sm p-5">
                  ...
                  <div className="flex gap-1">
                    {deletandoId === c.id ? (
                      <div className="flex items-center gap-1">
                        <button onClick={() => handleDelete(c.id)} className="text-xs text-zinc-900 dark:text-zinc-900 dark:text-white bg-red-500 hover:bg-red-600 px-2 py-1 rounded-lg">Sim</button>
                      ...
                      <>
                        <button onClick={() => openEdit(c)} className="p-1.5 hover:bg-zinc-50 dark:bg-zinc-800 rounded-lg text-zinc-400 dark:text-zinc-500 hover:text-zinc-400 dark:text-zinc-500 dark:hover:text-zinc-400 dark:text-zinc-500"><Pencil size={14} /></button>
                        <button onClick={() => setDeletandoId(c.id)} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-zinc-400 dark:text-zinc-500 hover:text-red-500"><Trash2 size={14} /></button>
                      </>
                    )}
                  </div>
                </div>
                ...
                <div className="mb-3">
                  ...
                  <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-1 text-right">{pct.toFixed(0)}% utilizado</p>
                </div>

                <div className="flex justify-between text-sm">
                  <span className="text-zinc-400 dark:text-zinc-500">Disponível</span>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editando ? 'Editar cartão' : 'Novo cartão'} maxWidth="max-w-md">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button type="submit" disabled={formLoading} className="flex-1 bg-primary-600 hover:bg-primary-700 text-zinc-900 dark:text-zinc-900 dark:text-white py-2.5 rounded-xl text-sm font-medium transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
