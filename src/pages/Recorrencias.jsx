...
const inputCls = 'w-full px-4 py-2.5 border border-zinc-300 dark:border-zinc-700 rounded-xl text-sm bg-zinc-800 dark:text-zinc-900 dark:text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500/40 focus:border-primary-500'
...
export default function Recorrencias() {
  ...
  function freqBadge(f) {
    ...
    return map[f] || 'bg-zinc-800 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400'
  }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-xl font-semibold text-zinc-900 dark:text-zinc-900 dark:text-white">Recorrências</h1>
          <p className="text-zinc-400 dark:text-zinc-500 text-sm mt-0.5">Lançamentos automáticos recorrentes</p>
        </div>
      ...
      <div className="bg-white dark:bg-white dark:bg-zinc-900 shadow-sm border border-zinc-100 dark:border-zinc-100 dark:border-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-100 dark:border-zinc-800 p-5 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button onClick={handleGerar} disabled={gerarLoading}
            className="bg-primary-600 hover:bg-primary-700 text-zinc-900 dark:text-zinc-900 dark:text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors disabled:opacity-60 flex items-center gap-2">
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-white dark:bg-zinc-900 shadow-sm border border-zinc-100 dark:border-zinc-100 dark:border-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-100 dark:border-zinc-800 p-16 text-center">
          <RefreshCw size={36} className="text-zinc-400 dark:text-zinc-500 dark:text-zinc-400 dark:text-zinc-500 mx-auto mb-3" />
          <p className="text-zinc-400 dark:text-zinc-500 font-medium">Nenhuma recorrência cadastrada</p>
          <p className="text-zinc-400 dark:text-zinc-500 text-sm mt-1">Crie recorrências para gerar lançamentos automaticamente.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {recorrencias.map(r => (
            <div key={r.id} className="bg-white dark:bg-white dark:bg-zinc-900 shadow-sm border border-zinc-100 dark:border-zinc-100 dark:border-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-100 dark:border-zinc-800 p-5">
              <div className="flex items-start justify-between gap-2 mb-3">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 truncate">{r.descricao}</p>
                  <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5">{r.categoriaNome || r.categoria?.nome || 'Sem categoria'}</p>
                </div>
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                ...
                {!r.ativa && (
                  <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-zinc-700 text-zinc-400 dark:text-zinc-500">
                    Inativa
                  </span>
                )}
              </div>
                    ...
                    {desativandoId === r.id ? (
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => handleDesativar(r.id)}
                          className="text-xs text-zinc-900 dark:text-zinc-900 dark:text-white bg-red-500 hover:bg-red-600 px-2 py-1 rounded-lg transition-colors">Sim</button>
                      ...
                      <button onClick={() => setDesativandoId(r.id)}
                        className="p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl text-zinc-400 dark:text-zinc-500 dark:text-zinc-400 dark:text-zinc-500 hover:text-red-500 transition-colors"
              ...
              {(r.dataInicio || r.dataFim) && (
                <div className="mt-3 pt-3 border-t border-zinc-200 dark:border-zinc-100 dark:border-zinc-800 text-xs text-zinc-400 dark:text-zinc-500 flex gap-3">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nova recorrência" maxWidth="max-w-lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">Dia referência (1–28)</label>
              <input type="number" required min="1" max="28" value={form.diaReferencia} onChange={set('diaReferencia')} className={inputCls} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">Data início</label>
              <input type="date" required value={form.dataInicio} onChange={set('dataInicio')} className={inputCls} />
            </div>
            <div>
              <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">Data fim (opcional)</label>
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button type="submit" disabled={formLoading}
              className="flex-1 bg-primary-600 hover:bg-primary-700 text-zinc-900 dark:text-zinc-900 dark:text-white py-2.5 rounded-xl text-sm font-medium transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
