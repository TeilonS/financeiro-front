  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-xl font-semibold text-zinc-900 dark:text-zinc-900 dark:text-white">Importar Extrato</h1>
      ...
      <div className="bg-white dark:bg-white dark:bg-zinc-900 shadow-sm border border-zinc-100 dark:border-zinc-100 dark:border-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-100 dark:border-zinc-800 p-6 mb-6">
        <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-700 dark:text-zinc-300 mb-4">Novo arquivo</h2>
          ...
          {FORMATOS.map(f => (
            <button key={f.value} onClick={() => setFormato(f.value)}
              className={`px-4 py-2 rounded-xl text-sm font-medium border transition-colors ${
                formato === f.value
                  ? 'bg-primary-600 text-zinc-900 dark:text-zinc-900 dark:text-white border-primary-600'
                  : 'border-zinc-300 dark:border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-700 dark:text-zinc-300 hover:bg-zinc-800'
              }`}>
              {f.label}
            </button>
          ))}
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer transition-all ${
            dragging
              ...
              : 'border-zinc-300 dark:border-zinc-200 dark:border-zinc-700 hover:border-primary-300 hover:bg-zinc-800'
          }`}
        >
          {uploadLoading ? (
            <div className="flex flex-col items-center gap-3">
              <Loader2 size={32} className="animate-spin text-primary-500" />
              <p className="text-sm text-zinc-400 dark:text-zinc-500">Processando arquivo...</p>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-zinc-700 dark:text-zinc-700 dark:text-zinc-300">Arraste ou clique para selecionar</p>
                <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-1">Formato selecionado: <strong>{FORMATOS.find(f => f.value === formato)?.label}</strong></p>
              </div>
              <button type="button"
                className="bg-primary-600 hover:bg-primary-700 text-zinc-900 dark:text-zinc-900 dark:text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors flex items-center gap-2"
        ...
        {uploadResult && (
          <div className="mt-5 space-y-3">
            <div className="bg-zinc-800 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-100 dark:border-zinc-800 rounded-xl px-4 py-3 text-sm flex flex-wrap gap-4">
              <span className="text-zinc-700 dark:text-zinc-700 dark:text-zinc-300">
                <strong className="text-zinc-400 dark:text-zinc-500 dark:text-zinc-900 dark:text-zinc-900 dark:text-white">{uploadResult.totalTransacoes}</strong> transações importadas
              </span>
      ...
      {selecionados.length > 0 && (
        <div className="bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800 rounded-2xl p-4 mb-4 flex flex-wrap items-center gap-3">
          ...
          <select
            ...
            className="flex-1 min-w-[180px] px-3 py-1.5 border border-primary-200 dark:border-primary-700 rounded-xl text-sm bg-zinc-800 dark:text-zinc-900 dark:text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500/40 focus:border-primary-500">
          ...
          <button onClick={handleConfirmarLote} disabled={!loteCategoria || loteLoading}
            className="bg-primary-600 hover:bg-primary-700 text-zinc-900 dark:text-zinc-900 dark:text-white px-4 py-1.5 rounded-xl text-sm font-medium transition-colors disabled:opacity-60 flex items-center gap-2">
      ...
      <div className="bg-white dark:bg-white dark:bg-zinc-900 shadow-sm border border-zinc-100 dark:border-zinc-100 dark:border-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-100 dark:border-zinc-800 p-6">
        <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-700 dark:text-zinc-300 mb-4">Pendentes anteriores</h2>
        {pendentesLoading ? (
          <div className="flex items-center justify-center h-24">
            <Loader2 size={24} className="animate-spin text-primary-500" />
          </div>
        ) : pendentesExist.length === 0 ? (
          <p className="text-zinc-400 dark:text-zinc-500 text-sm text-center py-8">Nenhuma transação pendente.</p>
        ) : (
          <PendentesList
        ...
        {confirmModal && (
          <div className="space-y-4">
            <div className="bg-zinc-800 dark:bg-zinc-800 rounded-xl p-4 text-sm">
              <p className="font-medium text-zinc-800 dark:text-zinc-800 dark:text-zinc-200">{confirmModal.descricao}</p>
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-2.5 border border-zinc-300 dark:border-zinc-200 dark:border-zinc-700 rounded-xl text-sm bg-zinc-800 dark:text-zinc-900 dark:text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500/40 focus:border-primary-500">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleConfirmar} disabled={confirmLoading}
                className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-zinc-900 dark:text-zinc-900 dark:text-white py-2.5 rounded-xl text-sm font-medium transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
        ...
        <div key={item.id} className="flex items-center justify-between py-3 gap-3">
          {onToggle && (
            <input type="checkbox" checked={selecionados.includes(item.id)} onChange={() => onToggle(item.id)}
              className="w-4 h-4 rounded border-zinc-300 dark:border-zinc-200 dark:border-zinc-700 text-primary-600 cursor-pointer shrink-0" />
          )}
          <div className="flex items-center gap-3 min-w-0">
            <div className={`w-2 h-2 rounded-full shrink-0 ${item.tipo === 'RECEITA' ? 'bg-emerald-400' : 'bg-red-400'}`} />
            <div className="min-w-0">
              <p className="text-sm font-medium text-zinc-800 dark:text-zinc-800 dark:text-zinc-200 truncate">{item.descricao}</p>
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <button onClick={() => onConfirmar(item)}
              className="bg-primary-600 hover:bg-primary-700 text-zinc-900 dark:text-zinc-900 dark:text-white px-3 py-1.5 rounded-lg text-xs font-medium transition-colors flex items-center gap-1">
