  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-xl font-semibold text-zinc-900 dark:text-zinc-900 dark:text-white">Metas</h1>
          <p className="text-zinc-400 dark:text-zinc-500 text-sm mt-0.5">Controle de orçamento por categoria</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-white dark:bg-zinc-900 shadow-sm border border-zinc-100 dark:border-zinc-100 dark:border-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-100 dark:border-zinc-800 p-16 text-center">
          <Target size={36} className="text-zinc-400 dark:text-zinc-500 dark:text-zinc-400 dark:text-zinc-500 mx-auto mb-3" />
          <p className="text-zinc-400 dark:text-zinc-500 font-medium">Nenhuma meta para este mês</p>
          <p className="text-zinc-400 dark:text-zinc-500 text-sm mt-1">Crie metas para controlar seus gastos por categoria.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {metasFiltradas.map(m => {
            ...
            return (
              <div key={m.id} className="bg-white dark:bg-white dark:bg-zinc-900 shadow-sm border border-zinc-100 dark:border-zinc-100 dark:border-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-100 dark:border-zinc-800 p-5">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">{m.categoriaNome || m.categoria?.nome || '—'}</p>
                    <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5">Limite: {fmt(m.valorLimite)}</p>
                  </div>
                  {deletandoId === m.id ? (
                    ...
                    <button
                      ...
                      className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-zinc-400 dark:text-zinc-500 dark:text-zinc-400 dark:text-zinc-500 hover:text-red-500 transition-colors"
                    >
                      <Trash2 size={14} />
                    </button>
                  )}
                ...
                <div className="mb-2">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-xs text-zinc-400 dark:text-zinc-500">Gasto: {fmt(m.valorGasto || 0)}</span>
