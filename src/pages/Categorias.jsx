        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={() => setModalOpen(false)}
              className="flex-1 border border-zinc-300 dark:border-zinc-700 hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300 py-2.5 rounded-xl text-sm font-medium">Cancelar</button>
...
function CategoryCard({ cat, deletandoId, setDeletandoId, handleDelete, isRoot }) {
  return (
    <div className={`bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-4 flex items-center justify-between group ${isRoot ? '' : 'py-3'}`}>
      <div className="flex items-center gap-3">
      ...
      {deletandoId === cat.id ? (
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={() => setDeletandoId(null)} className="text-xs text-zinc-700 dark:text-zinc-300 bg-zinc-700 hover:bg-zinc-800 px-2 py-1 rounded-lg">Não</button>
        </div>
      ) : (
