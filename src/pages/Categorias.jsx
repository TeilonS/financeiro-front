...
const inputCls = 'w-full px-4 py-2.5 border border-zinc-300 dark:border-zinc-200 dark:border-zinc-700 rounded-xl text-sm bg-zinc-800 dark:text-zinc-900 dark:text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500/40 focus:border-primary-500'
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-xl font-semibold text-zinc-900 dark:text-zinc-900 dark:text-white">Categorias</h1>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nova categoria">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-700 dark:text-zinc-300 mb-1.5">Nome</label>
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-700 dark:text-zinc-300 mb-1.5">
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-700 dark:text-zinc-300 mb-1.5">Cor</label>
            <div className="flex items-center gap-3">
              <input type="color" value={form.cor} onChange={set('cor')}
                className="w-12 h-10 rounded-lg border border-zinc-300 dark:border-zinc-200 dark:border-zinc-700 cursor-pointer p-1 dark:bg-zinc-800" />
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={() => setModalOpen(false)}
              className="flex-1 border border-zinc-300 dark:border-zinc-200 dark:border-zinc-700 hover:bg-zinc-800 text-zinc-700 dark:text-zinc-700 dark:text-zinc-300 py-2.5 rounded-xl text-sm font-medium">Cancelar</button>
            <button type="submit" disabled={formLoading}
              className="flex-1 bg-primary-600 hover:bg-primary-700 text-zinc-900 dark:text-zinc-900 dark:text-white py-2.5 rounded-xl text-sm font-medium disabled:opacity-60 flex items-center justify-center gap-2">
          ...
          <div key={c.id}>
            ...
            {subcats.filter(s => s.categoriaPaiId === c.id).map(s => (
              <div key={s.id} className="ml-6 mt-1.5 flex items-center gap-2">
                <ChevronRight size={12} className="text-zinc-400 dark:text-zinc-500 dark:text-zinc-400 dark:text-zinc-500 shrink-0" />
...
function CategoryCard({ cat, deletandoId, setDeletandoId, handleDelete, isRoot }) {
  return (
    <div className={`bg-white dark:bg-white dark:bg-zinc-900 shadow-sm border border-zinc-100 dark:border-zinc-100 dark:border-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-100 dark:border-zinc-800 p-4 flex items-center justify-between group ${isRoot ? '' : 'py-3'}`}>
      <div className="flex items-center gap-3">
        <div className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: cat.cor || '#EF4444' }} />
        <div>
          <p className="text-sm font-medium text-zinc-800 dark:text-zinc-800 dark:text-zinc-200">{cat.nome}</p>
          <span className={`text-xs font-medium ${cat.tipo === 'RECEITA' ? 'text-emerald-600' : 'text-red-500'}`}>{cat.tipo}</span>
        </div>
      </div>
      {deletandoId === cat.id ? (
        <div className="flex items-center gap-1.5">
          <button onClick={() => handleDelete(cat.id)} className="text-xs text-zinc-900 dark:text-zinc-900 dark:text-white bg-red-500 hover:bg-red-600 px-2 py-1 rounded-lg">Sim</button>
          <button onClick={() => setDeletandoId(null)} className="text-xs text-zinc-700 dark:text-zinc-700 dark:text-zinc-300 bg-zinc-700 hover:bg-zinc-800 px-2 py-1 rounded-lg">Não</button>
        </div>
      ) : (
