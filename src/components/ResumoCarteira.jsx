...
function PieTooltip({ active, payload }) {
  ...
  return (
    <div className="bg-white dark:bg-[#111111] rounded-xl shadow-2xl border border-zinc-100 dark:border-white/5 px-3 py-2 text-sm backdrop-blur-md">
      <p className="text-zinc-900 dark:text-white font-semibold">{brl(value)}</p>
      <p className="text-zinc-500 dark:text-zinc-400 text-xs">{name}</p>
    </div>
  );
}
...
export default function ResumoCarteira({ investimentos = [] }) {
  ...
  if (total <= 0) {
    return (
      <div className="bg-white dark:bg-zinc-900 rounded-2xl p-5 border border-zinc-200 dark:border-white/5 shadow-sm text-zinc-500 dark:text-zinc-400 text-sm">
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl p-5 border border-zinc-200 dark:border-white/5 shadow-sm space-y-6">
      <div>
        <h3 className="text-xs font-semibold tracking-widest text-zinc-500 dark:text-zinc-400 mb-3">ALOCAÇÃO POR TIPO</h3>
        <div className="flex flex-col sm:flex-row items-center gap-4">
          <div className="relative w-40 h-40 shrink-0">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              ...
              <span className="text-sm font-semibold text-zinc-900 dark:text-white">{brl(total)}</span>
            </div>
          </div>
          <div className="flex-1 w-full space-y-2">
            {alocacao.map((a) => (
              <div key={a.tipo} className="flex items-center justify-between text-sm">
                ...
                <span className="text-zinc-500 dark:text-zinc-400">{pct(a.peso)} · {brl(a.valor)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div>
        <h3 className="text-xs font-semibold tracking-widest text-zinc-500 dark:text-zinc-400 mb-3">PESO POR ATIVO</h3>
        <div className="space-y-3">
          {ativos.map((a, idx) => (
            <div key={idx}>
              <div className="flex justify-between text-sm mb-1">
                <span className="text-zinc-700 dark:text-zinc-200 truncate pr-2">{a.nome}</span>
                <span className="text-zinc-500 dark:text-zinc-400 shrink-0">{pct(a.peso)}</span>
              </div>
